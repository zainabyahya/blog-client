        ...
        <div className='flex'>
            {posts.length > 0 && (
                <div className='w-full m-5'>
                    <div style={{ width: "100%", height: "700px", objectFit: "cover", overflow: "hidden" }}
                        className=''>
                        <img style={{ height: "auto" }} src={`http://localhost:8000/${posts[selectedPostIndex].image}`} alt='Blog Cover' />
                    </div>
                    <div className='w-full flex flex-col-reverse md:flex-row'>
                        <div id='choose' className='md:w-1/2 flex justify-center gap-2 md:flex-col items-end md:justify-end p-5'>
                            {posts.map((_, index) => (
                                <button
                                    key={index}
                        ...
                        <div className='md:w-1/2 bg-[#e5e5e5] p-10 flex flex-col gap-4 text-black' onClick={() => handlePostClick(posts[selectedPostIndex]._id)}>
